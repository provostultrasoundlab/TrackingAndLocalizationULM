%% File: PALA_TAL_InVivoULM_example.m
% Description: This script performs post-processing operations such as filtering, 
% localization, and tracking on in vivo data. It is an adaptation of the script 
% PALA_InVivoULM_example.m, which can be found in the PALA repository on GitHub 
% (https://github.com/AChavignon/PALA).
%
% The key difference in this script is the replacement of the localization and 
% tracking methodologies by the Tracking and Localization (TAL) approach. 
% More details about the TAL approach can be found in the following paper: 
% https://arxiv.org/abs/2308.02724
...
cd ../
addpath(genpath(pwd)) 
PALA_data_folder = 'E:\'; % Change the path here to the PALA_data_InVivoRatBrain dataset
...
ParamProcessing = dataLoader.LoadSaveParams(cfg_processing_path,[workingdir filesep filename,'_cfg_processing.json']);
name_tracker = ParamProcessing.trackingMethod{1};
...
% for hhh = 1:Nbuffers
    ...
    resultsTracking  =SpatioTempTracker(IQ_filt,ParamProcessing);
%     resultsTracking_out.(name_tracker)=resultsTracking;
    tracks = Tracks(resultsTracking,ParamProcessing,framerate,SizeOfBloc(3));
    tracks= Interpolator.interp_tracks(tracks);
    tracks = Velocitor.velocity_tracks(tracks);
    Track_tot_i = tracks.tracks_interp.(name_tracker);
    Track_tot_i = cellfun(@(x) x(:, [3, 1, 7, 5, 4]), Track_tot_i, 'UniformOutput', false);
    Track_tot_i = cellfun(@(x) [(x(:,[1,2]) -[0.5 0.5]).*PData.PDelta([3 1]) + [PData.Origin(3) PData.Origin(1)],x(:,3:end) ], Track_tot_i, 'UniformOutput', false);
    % Saving part:
...
save([workingdir filesep filename 'example_TAL_tracks.mat'],'Track_tot_1','Track_tot_2','Track_tot_3','Tend','ULM','-v6')
...
save([workingdir filesep filename 'example_TAL_matouts.mat'],'MatOut','MatOut_zdir','MatOut_vel','ULM','lambda','llx','llz')
...
print(gcf,[workingdir filesep filename 'example_TAL_MatOut'],'-dpng','-r750')
WriteTif(im.CData,ca.Colormap,[workingdir filesep filename 'example_TAL_MatOut.tif'],'caxis',caxis,'Overwrite',1)
...
print(gcf,[workingdir filesep filename 'example_TAL_MatOut_zdir'],'-dpng','-r750')
WriteTif(im.CData,ca.Colormap,[workingdir filesep filename 'example_TAL_MatOut_zdir.tif'],'caxis',caxis,'Overwrite',1)
...
print(gcf,[workingdir filesep filename 'example_TAL_VelMorm'],'-dpng','-r750')
imwrite(Mvel_rgb,[workingdir filesep filename 'example_TAL_VelMorm.tif'])
...
print(gcf,[workingdir filesep filename 'example_TAL_PowDop'],'-dpng','-r750')
WriteTif(im.CData,ca.Colormap,[workingdir filesep filename 'example_TAL_PowDop.tif'],'caxis',caxis,'Overwrite',1)

fprintf('PALA_TAL_InVivoULM_example.m completed.\n');