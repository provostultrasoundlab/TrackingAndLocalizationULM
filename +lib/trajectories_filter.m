function Vfiltered = trajectories_filter(I, options)
% This function is the same as the vesselness3D function from the JermanEnhancementFilter library
% exepct there is an additional input parameter "epsilon" 
% calculates vesselness probability map (local tubularity) of a 3D input 
% image
% 
% vesselness = vesselness3D(V, sigmas, spacing, tau, brightondark,epsilon)
% 
% inputs,
%   I : 3D image
...
%       dimensions 
%   tau : (between 0 and 1) : parameter that controls response uniformity
...
%   epsilon: > 1 Improved filtering of trajectories oriented along the time dimension
%
% outputs,
%   Vfiltered: maximum vesselness response over scales sigmas
% Function was adapted by A.Leconte (2024)
% The original function was written by T. Jerman, University of Ljubljana (October 2014)
% Based on code by D. Kroon, University of Twente (May 2009)
addpath(genpath('JermanEnhancementFilter'));
verbose = 1;
defaultoptions = struct('sigmas', 4, 'spacing', [1;1;1], 'tau', 0.5, 'brightondark', true, 'epsilon', 1.4);
if nargin < 2
...
tau = options.tau;
brightondark = options.brightondark;
...
for j = 1:length(sigmas)
    ...
    [~, Lambda2, Lambda3] = volumeEigenvalues(I,sigmas(j),spacing,brightondark,epsilon);
    ...
    if(j==1)
        ...
        Vfiltered = max(Vfiltered,response);
    end
...
Vfiltered = Vfiltered ./ max(Vfiltered(:));
Vfiltered(Vfiltered < 1e-2) = 0;    


function [Lambda1, Lambda2, Lambda3] = volumeEigenvalues(V,sigma,spacing,brightondark,epsilon)
...
% Correct for scaling
c=sigma.^2;
Hxx = c*Hxx*(epsilon^2); Hxy = c*Hxy*(epsilon^2);
Hxz = c*Hxz*epsilon; Hyy = c*Hyy*(epsilon^2);
Hyz = c*Hyz*epsilon; Hzz = c*Hzz;
